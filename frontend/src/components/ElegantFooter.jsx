...
const ElegantFooter = ({ theme = 'gold' }) => {
  const themes = {
    gold: {
  ...
  const services = ['Wealth Management', 'Investment Advisory', 'Family Office', 'Portfolio Management', 'Tax Planning', 'Estate Planning'];
            ...
            <div className="flex space-x-3">
              {['linkedin', 'twitter', 'instagram'].map((social, i) => (
                <motion.a
                  ...
                  className={`w-10 h-10 rounded-full border ${t.border} flex items-center justify-center text-gray-500 ${t.accentHover} transition-colors`}
                >
          ...
          <div>
            ...
            <ul className="space-y-3">
              {quickLinks.map((item, index) => (
                <li key={index}>
                  <Link to={`/${item.toLowerCase().replace(' ', '-')}`} className={`text-gray-600 text-sm ${t.accentHover} transition-colors`}>
                    {item}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {services.map((item, index) => (
                <li key={index}>
                  <Link to="/services" className={`text-gray-600 text-sm ${t.accentHover} transition-colors`}>
                    {item}
                  </Link>
                </li>
              ))}
